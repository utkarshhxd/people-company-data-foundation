"""Consume schema.mapped and normalize that batch's records.

Offsets commit only after a batch is fully written (at-least-once).
Reprocessing is safe: observations carry a uniqueness constraint on
(record_id, source_column, value_index) and inserts are ON CONFLICT DO NOTHING.
"""
...
CONSUMER_GROUP = "pcdf-normalization"
...
def _handle(payload: dict, producer: EventProducer) -> None:
    ...
    if not batch_id:
        raise BatchNotNormalizable(f"event has no batch_id: {payload}")

    result = normalize_batch(batch_id)
    producer.publish(
        events.TOPIC_RECORDS_NORMALIZED,
        key=result.batch_id,
        event=events.records_normalized(
...
def run() -> int:
    configure("normalization-consumer")
    ...
    consumer.subscribe([events.TOPIC_SCHEMA_MAPPED])
    logger.info("consuming %s as group %s", events.TOPIC_SCHEMA_MAPPED, CONSUMER_GROUP)

    heartbeat = default_path("normalization-consumer")
    try:
        while _running:
