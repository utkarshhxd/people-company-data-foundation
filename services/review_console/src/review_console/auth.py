...
**It fails closed on the thing that matters.** The console shows personal
data with full provenance attached -- names, home addresses, phone numbers,
raw quarantined rows, which vendor sold them. `PCDF_API_KEYS` unset means no
keys are configured; the console then serves only on loopback and says so
loudly at startup. Anything else is refused rather than silently served,
because an unauthenticated deployment that *looks* fine is exactly how this
leaks.
...
# console is reachable from another host, it is reachable from the network, and
...
def describe_configuration() -> str:
    ...
    if unauthenticated_allowed():
        return (
            f"The console is UNAUTHENTICATED: {ENV_ALLOW_UNAUTH} is set. Anyone who "
            "can reach this port can read every record and its provenance."
        )
    return (
