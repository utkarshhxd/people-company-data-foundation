"""What the API does when nobody has configured a key, and when somebody has."""

import pytest
from api_service.auth import ENV_ALLOW_UNAUTH, ENV_KEYS
...
def _stub_lineage(monkeypatch):
    """The queries are not under test here; the gate in front of them is."""
    monkeypatch.setattr(lineage, "find_entities", lambda *a, **k: [])
...
def test_loopback_is_allowed_with_no_keys():
    ...
    response = client("127.0.0.1").get("/entities")
...
def test_off_box_is_refused_with_no_keys():
    ...
    """
    response = client("10.0.0.5").get("/entities")
...
def test_unauthenticated_can_be_chosen_deliberately(monkeypatch):
    ...
    assert client("10.0.0.5").get("/entities").status_code == 200
...
def test_a_valid_key_is_accepted(monkeypatch):
    ...
    response = client().get("/entities", headers={"X-API-Key": "secret-one"})
    assert response.status_code == 200


def test_bearer_is_accepted_too(monkeypatch):
    monkeypatch.setenv(ENV_KEYS, "secret-one")
    response = client().get("/entities", headers={"Authorization": "Bearer secret-one"})
    assert response.status_code == 200
...
def test_any_configured_key_works(monkeypatch):
    ...
    for key in ("old-key", "new-key"):
        assert client().get("/entities", headers={"X-API-Key": key}).status_code == 200
...
def test_a_wrong_key_is_rejected(monkeypatch):
    monkeypatch.setenv(ENV_KEYS, "secret-one")
    assert client().get("/entities", headers={"X-API-Key": "nope"}).status_code == 401


def test_loopback_does_not_bypass_a_configured_key(monkeypatch):
    """The local exemption covers an unconfigured API, never a configured one.
