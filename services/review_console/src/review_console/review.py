"""Read projections for the four review queues.

Four stages in this pipeline stop and ask a person: schema mapping when a
column is ambiguous, entity resolution when a match is plausible but not
certain, validation when a record fails outright, and AI enrichment when a
local model guesses a field no vendor reported. Each already has a CLI. What
none of them had is a way to *see the thing being decided* -- the sample
values under an ambiguous column, the two entities a merge would join, the
failures that put a record in quarantine, the known values around an AI
guess -- without running several commands and joining the output by eye.
...
made in a browser and the same decision made in a terminal are the same code
path. Nothing here writes, and nothing here reimplements a rule.
"""
...
from enrichment.repository import known_values as enrichment_known_values
...
SELECT
  ...
  (SELECT count(*) FROM enrichment_proposal WHERE status = 'pending')
      AS enrichment_open,
  (SELECT coalesce(extract(epoch FROM now() - min(created_at)), 0)
     FROM enrichment_proposal WHERE status = 'pending')
      AS enrichment_oldest_seconds
"""


def queue_summary() -> dict[str, Any]:
    """Depth and age of all four queues.
    ...
    queues = {
        ...
        QUEUE_ENRICHMENT: queue("enrichment"),
    }
    return {
        "queues": queues,
...
def enrichment_queue(
    ...
    """AI guesses waiting on a human, each with the entity's other known values.

    The known values are what a reviewer actually judges a guess against -- a
    proposed job title next to the company and location already on file is
    judgeable; the same title alone is not.
    """
    with connect() as conn:
        rows = list_proposals(conn, status=status, entity_type=entity_type)
        out = []
            ...
            out.append(
                {
                    ...
                    "stated_confidence": float(row["stated_confidence"]),
                    "model": row["model"],
                    "status": row["status"],
                    "reviewed_by": row["reviewed_by"],
                    "created_at": row["created_at"],
                    "known_values": enrichment_known_values(conn, entity_id),
                }
            )
    return out
