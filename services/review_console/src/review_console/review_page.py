...
It exists so the four review queues can be worked by someone who is not at a
terminal, which until now they could not be. It is deliberately one
self-contained document served by this service rather than a frontend
application: giving it a toolchain would be more infrastructure than the job
needs, and the job is showing a person the thing they have to decide about.
  ...
  return fetch(path, Object.assign({ headers: headers() }, options || {}))
    .then(function (res) {
      if (res.status === 401 || res.status === 403) {
        ...
        throw new Error("This console needs a key. Enter it above and refresh.");
      }
